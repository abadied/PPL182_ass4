...
export const typeofExps = (exps: Exp[], tenv: TEnv): TExp | Error =>
    ...
    isDefineExp(first(exps)) ? isError(typeofDefine(first(exps), tenv)) ? typeofDefine(first(exps), tenv) : typeofExps(rest(exps),makeExtendTEnv([first(exps).var.var],[first(exps).var.texp],tenv)) :
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): TExp | Error => {
    ...
    const constraint =  checkEqualType(varTE, typeofExp(val, tenv), exp);
                                
    if (isError(constraint))
    return Error(getErrorMessages([constraint]));
    else
        return makeVoidTExp();
};
...
export const typeofProgram = (exp: Program, tenv: TEnv): TExp | Error => {
    ...
    return isError (constraint) ? Error("Program gone bad!") : constraint;
};
